							...
							>
								...
								<template slot="items" slot-scope="props">
									...
									<td class="text-xs-left">
										{{ props.item.Linkedaccount.linkedaccount_accountDisplayName }}
										({{ props.item.Linkedaccount.linkedaccount_currency }})
									</td>
									<td class="text-xs-left">{{ props.item.Linkedaccount.linkedaccount_password || '*********' }}</td>
									<td class="text-xs-left">{{ props.item.connection }}</td>
									<td class="text-xs-left">
										<div :class="{
											'ready' : props.item.Linkedaccount.linkedaccount_visualStatus === 'MONITORED',
											'pending' : props.item.Linkedaccount.linkedaccount_visualStatus === 'ANALYZING',
											'waiting' : props.item.Linkedaccount.linkedaccount_visualStatus === 'QUEUED' }"
											class="td-actions"
										>
											<v-icon class="point">fiber_manual_record</v-icon>
											{{ props.item.Linkedaccount.linkedaccount_visualStatus }}
										</div>
									</td>
									<td class="text-xs-left">
										<div class="td-actions">
											<i class="icon-win-trash" @click="deleteLinkedAccount(props.item.links)"></i>
											<i class="icon-win-edit" @click="editLinkedAccount(props.item.links)"></i>
										</div>
									</td>										
								</template>
							</v-data-table>								
			...
			<v-card class="card-modal custom-checks">
				<div class="title-modal-confirm">Select your investment account</div>
					<v-checkbox
						...
						:label="item.linkedaccount_platform_display_name"
						color="light-blue accent-3"
						hide-details
						:value="item"
						multiple
						v-for="item in accounts" :key="item.linkedaccount_identity"
					></v-checkbox>
				<div class="buttons">
...
<script>
	...
	export default {
		...
		methods: {
			...
			deleteLinkedAccount (item) {
				console.log(item[1].href)
				axios.delete(`/api/link/?href=${item[1].href}`)
					.then(({ data }) => {
						console.log(data)
					})
			},
			editLinkedAccount (item) {
				console.log(item)
			},
			analizeAccount (user) {
				setTimeout(() => {
